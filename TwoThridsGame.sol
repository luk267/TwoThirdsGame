...
contract TwoThirdsGame is ReentrancyGuard {
    ...
    function register() external payable nonReentrant {
        ...
        if (_playerIdCounter.current() >= minPlayers && block.timestamp >= registrationDeadline) {
    ...
    function advanceToSubmission() external {
        ...
        require(_playerIdCounter.current() >= minPlayers, "Not enough players");
    ...
    function revealNumber(uint256 number, string calldata secret) external {
        ...
        for (uint256 i = 1; i <= _playerIdCounter.current(); i++) {
    ...
    function cancelGame() external {
        ...
        require(_playerIdCounter.current() < minPlayers, "Enough players registered");
    ...
    function getPlayerCount() external view returns (uint256) {
        return _playerIdCounter.current();
    }
